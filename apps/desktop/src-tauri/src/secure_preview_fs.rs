...
use windows::Win32::Storage::FileSystem::{CreateFileW, GetFileInformationByHandle, SetFileInformationByHandle, BY_HANDLE_FILE_INFORMATION, DELETE, FILE_ATTRIBUTE_REPARSE_POINT, FILE_DISPOSITION_INFO, FILE_FLAGS_AND_ATTRIBUTES, FILE_GENERIC_READ, FILE_GENERIC_WRITE, FILE_SHARE_DELETE, FILE_SHARE_READ, FILE_SHARE_WRITE, FileDispositionInfo, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, FILE_FLAG_OPEN_REPARSE_POINT};
use windows::Win32::System::IO::IO_STATUS_BLOCK;
use windows::Wdk::Foundation::OBJECT_ATTRIBUTES;
use windows::Wdk::Storage::FileSystem::{NtCreateFile, FILE_DIRECTORY_FILE, FILE_NON_DIRECTORY_FILE, FILE_OPEN_IF, FILE_OPEN_REPARSE_POINT, FILE_SYNCHRONOUS_IO_NONALERT};

const MAX_BACKUP_BYTES: u64 = 64 * 1024 * 1024;
const FILE_CREATED: usize = 2;

fn invalid(message: &str) -> std::io::Error { std::io::Error::new(std::io::ErrorKind::InvalidInput, message) }

fn segments(relative: &str) -> std::io::Result<Vec<&str>> {
    if relative.is_empty() || relative.contains('\\') { return Err(invalid("unsafe preview path")); }
    let values = relative.split('/').collect::<Vec<_>>();
    if values.iter().any(|part| part.is_empty() || *part == "." || *part == ".." || part.contains(':')) {
        return Err(invalid("unsafe preview path"));
    }
    Ok(values)
}

fn checked_not_reparse(handle: HANDLE) -> std::io::Result<()> {
    let mut info = BY_HANDLE_FILE_INFORMATION::default();
    unsafe { GetFileInformationByHandle(handle, &mut info).map_err(|error| std::io::Error::other(error.message().to_string()))?; }
    if info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT.0 != 0 { return Err(invalid("reparse point rejected")); }
    Ok(())
}

fn checked_directory(handle: HANDLE) -> std::io::Result<()> { checked_not_reparse(handle) }
...
fn relative_open(parent: &File, name: &str, directory: bool, create: bool) -> std::io::Result<(File, bool)> {
    let mut wide = OsStr::new(name).encode_wide().collect::<Vec<_>>();
    let byte_len = wide.len().checked_mul(2).filter(|size| *size <= u16::MAX as usize).ok_or_else(|| invalid("path segment too long"))?;
    let mut unicode = UNICODE_STRING { Length: byte_len as u16, MaximumLength: byte_len as u16, Buffer: PWSTR(wide.as_mut_ptr()) };
    ...
    let access = windows::Win32::Storage::FileSystem::FILE_ACCESS_RIGHTS(FILE_GENERIC_READ.0 | FILE_GENERIC_WRITE.0 | DELETE.0);
    let options = if directory { FILE_DIRECTORY_FILE } else { FILE_NON_DIRECTORY_FILE }
        | FILE_OPEN_REPARSE_POINT | FILE_SYNCHRONOUS_IO_NONALERT;
    let disposition = if create { FILE_OPEN_IF } else { windows::Wdk::Storage::FileSystem::FILE_OPEN };
    let result = unsafe { NtCreateFile(&mut handle, access, &attributes, &mut status, None, FILE_FLAGS_AND_ATTRIBUTES(0), FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, disposition, options, None, 0) };
    if result.0 < 0 { return Err(std::io::Error::other(format!("NtCreateFile failed: 0x{:08x}", result.0 as u32))); }
    checked_not_reparse(handle)?;
    let created = status.Information == FILE_CREATED;
    Ok((unsafe { File::from_raw_handle(handle.0) }, created))
}
...
fn open_file(root: &File, relative: &str, create: bool) -> std::io::Result<(File, bool)> {
    ...
    let (name, parents) = parts.split_last().ok_or_else(|| invalid("preview path missing filename"))?;
...
fn copy_handle_to(root: &File, source: &mut File, backup_relative: &str) -> std::io::Result<()> {
    ...
    loop {
        ...
        if count == 0 { break; }
        total = total.checked_add(count as u64).ok_or_else(|| invalid("backup too large"))?;
        if total > MAX_BACKUP_BYTES { return Err(invalid("backup too large")); }
...
pub fn materialize(root_path: &Path, files: &[(String, Vec<u8>)], backup_prefix: &str) -> std::io::Result<usize> {
    materialize_with_before_first(root_path, files, backup_prefix, || {})
}

fn materialize_with_before_first<F: FnOnce()>(root_path: &Path, files: &[(String, Vec<u8>)], backup_prefix: &str, before_first: F) -> std::io::Result<usize> {
    ...
    for (relative, content) in files {
        let (mut destination, created) = match open_file(&root, relative, true) {
            Ok(value) => value,
            Err(error) => { rollback(&root, &applied); return Err(error); }
        };
        let backup = if created { None } else {
            let backup_relative = format!("{backup_prefix}/{relative}");
            if let Err(error) = copy_handle_to(&root, &mut destination, &backup_relative) { rollback(&root, &applied); return Err(error); }
            Some(backup_relative)
        };
        applied.push((relative.clone(), backup));
        let write = (|| { destination.set_len(0)?; destination.seek(SeekFrom::Start(0))?; destination.write_all(content)?; destination.sync_all() })();
        if let Err(error) = write { rollback(&root, &applied); return Err(error); }
    }
    Ok(applied.len())
}
...
fn rollback(root: &File, applied: &[(String, Option<String>)]) {
    for (relative, backup) in applied.iter().rev() {
        if let Some(backup) = backup { let _ = restore_from(root, relative, backup); }
...
mod tests {
    ...
    fn rejects_a_junction_replaced_after_the_root_handle_is_opened() {
        let nonce = format!("{}-{}", std::process::id(), SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos());
        let root = std::env::temp_dir().join(format!("blogbot-secure-preview-root-{nonce}"));
        let external = std::env::temp_dir().join(format!("blogbot-secure-preview-external-{nonce}"));
        ...
        let result = materialize_with_before_first(
            &root,
            &[("articles/example.md".to_owned(), b"outside write forbidden".to_vec())],
    ...
    fn writes_a_nested_preview_and_preserves_the_prior_file_in_the_safe_backup_tree() {
        ...
        assert_eq!(written, 1);
        assert_eq!(std::fs::read(root.join("articles/example.md")).unwrap(), b"new content");
        assert_eq!(std::fs::read(root.join(".blogbot/backups/test/articles/example.md")).unwrap(), b"old content");
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn restores_an_existing_file_when_a_later_bundle_entry_is_invalid() {
        ...
        assert!(result.is_err());
        assert_eq!(std::fs::read(root.join("articles/example.md")).unwrap(), b"old content");
        let _ = std::fs::remove_dir_all(&root);
    }
    ...
    fn removes_a_new_preview_file_when_a_later_bundle_entry_is_invalid() {
        let nonce = format!("{}-{}", std::process::id(), SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos());
        let root = std::env::temp_dir().join(format!("blogbot-secure-preview-new-rollback-{nonce}"));
        std::fs::create_dir_all(root.join("articles")).unwrap();
